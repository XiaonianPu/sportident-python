...
#                       2019  Per Magnusson <per.magnusson@gmail.com>
#                       2023  Xiaonian Pu <sserxiaonian@gmail.com>
#
...
def i2b(val: int, length: int = 1) -> bytes:
    """ simple unsigned int 2 bytes """
    return val.to_bytes(length, 'big')


def b2i(val: bytes) -> int:
    """ simple bytes 2 unsigned int """
    if len(val) == 0:
        raise ValueError
    else:
...
class SIReader(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._serial: Serial | None = None  # Serial port object
    ...
    def guessSerialPorts(self, ttyS=False):
        ...
        if sys.platform.startswith('linux'):
            ...
            # Sort on ranking
            ranked_ports.sort(key=lambda port: -port[0])
    ...
    def set_operating_mode(self, mode: int):
        ...
        try:
            self._send_command(SIReader.C_SET_SYS_VAL, SIReader.O_MODE + mode.to_bytes(1, 'little'))
        finally:
    ...
    def sysval_model_str(self):
        ...
        return SIReader.MODEL2NAME[model_id] if model_id in SIReader.MODEL2NAME else "0x%04x" % model_id
    ...
    def set_feedback(self, audible=True, optical=True):
        ...
        if audible:
            ...
            feedback &= ~0b00000100
        self._send_command(SIReader.C_SET_SYS_VAL, SIReader.O_FEEDBACK + i2b(feedback))
    ...
    def get_time(self):
        ...
        minute = second // 60
        second %= 60
        ms = int(round(bintime[6] / 256.0 * 1000000))
        try:
    ...
    def set_time(self, time):
        ...
        """
        bintime = (i2b(int(time.strftime('%y')), 1)
                   + i2b(time.month, 1)
                   + i2b(time.day, 1)
                   + i2b(((time.isoweekday() % 7) << 1) + time.hour // 12, 1)
                   + i2b((time.hour % 12) * 3600 + time.minute * 60 + time.second, 2)
                   + i2b(int(round(time.microsecond / 1000000.0 * 256)), 1)
                   )
    ...
    def beep(self, count=1):
        ...
        """
        self._send_command(SIReader.C_BEEP, i2b(count))
    ...
    def read_backup(self, progress=0):
        ...
        offs1 = b2i(SIReader.O_BACKUP_PTR_HI) + 1
        offs2 = b2i(SIReader.O_BACKUP_PTR_LO) + 1
        ...
        while read_ptr < end_ptr:
            read_ptr_bytes = (i2b(read_ptr >> 16) +
                              i2b((read_ptr >> 8) & 0xff) +
                              i2b(read_ptr & 0xff))
            byte_cnt = b'\x80'
            if end_ptr - read_ptr < 0x80:
                byte_cnt = i2b(end_ptr - read_ptr)
        ...
        while ii < len(bakmem):
            ...
            if self.proto_config['ext_proto']:
                ...
                if punch[SIReader.BUX_SECS] >= 0xF0:
                    # Error code
                    err = "Err%X" % (punch[SIReader.BUX_SECS] & 0xF)
                else:
                ...
                # Legacy protocol
                cardnr_bytes = (b'\x00' + punch[SIReader.BUL_CNS:SIReader.BUL_CNS + 1] +
                ...
                if punch[SIReader.BUL_SECS] >= 0xF0:
                    # Error code
                    err = "Err%X" % (punch[SIReader.BUL_SECS] & 0xF)
                else:
    ...
    def _update_proto_config(self):
        ...
        if self._noconnect:
            ...
            config_byte = b2i(SIReader._extract_sysval(sysval, SIReader.O_PROTO, 1))
            ...
            mode_byte = b2i(SIReader._extract_sysval(sysval, SIReader.O_MODE, 1))
    ...
    def _set_proto_config(self, config):
        try:
            config_byte = i2b((config['ext_proto'] << 0) |
                              (config['auto_send'] << 1) |
                              (config['handshake'] << 2) |
                              (config['pw_access'] << 4) |
                              (config['punch_read'] << 7))
    ...
    def _decode_carddata(data, card_type, reftime=None):
        ...
        punch_count = b2i(data[card['RC']:card['RC'] + 1])
        if card_type == 'SI5':
    ...
    def _send_command(self, command, parameters, **kw):
        try:
            ...
            command_string = command + i2b(len(parameters)) + parameters
            ...
            if self._debug:
                print("==>> command '%s', parameters %s, crc %s" %
                      (hexlify(command).decode('ascii'),
                       ' '.join([hexlify(i2b(c)).decode('ascii') for c in parameters]),
    ...
    def _read_command(self, timeout=None):
        ...
        try:
            ...
            if char == b'':
                ...
                self._serial.flushInput()
                raise SIReaderException('Invalid start byte %s' % hex(char[0]))
            ...
            data = self._serial.read(int.from_bytes(length, 'big') - 2)
            ...
            if self._debug:
                print("<<== command '%s', len %i, station %s, data %s, crc %s, etx %s" %
                      (hexlify(cmd).decode('ascii'),
                       ...
                       ' '.join([hexlify(i2b(c)).decode('ascii') for c in data]),
                       hexlify(crc).decode('ascii'),
    ...
    @staticmethod
    def _extract_sysval(data: bytes, offset: bytes, length: int) -> bytes:
        """
        Extract a piece of the return data from the command C_GET_SYS_VAL '\x00\x80'.
        @param data: The data bytes from _read_command()
...
class SIReaderReadout(SIReader):
    ...
    def read_sicard_raw(self) -> (bytes, str):
        """Reads out the SI Card currently inserted into the station. The card must be
        detected with poll_sicard before."""
    ...
    def _read_command(self, timeout=None):
        ...
        here."""
        cmd, data = super()._read_command(timeout)
        ...
        if cmd == SIReader.C_SI_REM:
            ...
            if 2000000 <= self.sicard <= 2999999:
                self.cardtype = 'SI8'
            elif 1000000 <= self.sicard <= 1999999:
                self.cardtype = 'SI9'
            elif 4000000 <= self.sicard <= 4999999:
                self.cardtype = 'pCard'
            #            elif self.sicard >= 6000000 and self.sicard <= 6999999:  # tCard, don't have one for testing
            #                self.cardtype = 'SI9'
            elif 7000000 <= self.sicard <= 9999999:
                self.cardtype = 'SI10'
            else:
...
class SIReaderControl(SIReader):
    ...
    def poll_punch(self, timeout=0):
        ...
        while True:
            try:
                cmd, data = self._read_command(timeout=timeout)
            except SIReaderTimeout:
                break

            if cmd == SIReader.C_TRANS_REC:
                cur_offset = SIReader._to_int(data[SIReader.T_OFFSET:SIReader.T_OFFSET + 3])
                ...
                raise SIReaderException('Unexpected command %s received' % hex(b2i(cmd)))
            punches.append((self._decode_cardnr(data[SIReader.T_CN:SIReader.T_CN + 4]),
                            self._decode_time(data[SIReader.T_TIME:SIReader.T_TIME + 2])))
        return punches
    ...
    def _read_punch(self, offset):
        ...
        c = self._send_command(SIReader.C_GET_BACKUP,
                               i2b(offset, 3) + i2b(SIReader.REC_LEN))
